...
EARLY_WALLS = [
    [0, 13], [1, 12], [2, 11], [3, 10], [4, 9],
    [5, 8], [6, 7], [7, 6], [8, 5], [9, 4],
]

FLEX_BUILD_SEQUENCE = [
    [10, 3], [11, 2], [12, 2], [13, 2], [14, 2],
    [15, 3], [16, 4], [17, 5], [18, 6], [19, 7],
]

CORE_UPGRADE_SEQUENCE = [
    [11, 2], [12, 2], [13, 2], [14, 2],
]

SUPPORT_BUILD_UPGRADE_QUEUE = [
    [14, 3], [13, 3], [12, 3], [11, 3], [15, 4],
...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def build_image_layout(self, game_state):
        ...
        self.spawn_in_order(game_state, WALL, EARLY_WALLS)
        self.build_flex_sequence(game_state)
        if self.is_initial_layout_complete(game_state):
            self.upgrade_in_order(game_state, CORE_UPGRADE_SEQUENCE)
            self.build_and_upgrade_support_queue(game_state)

    def build_flex_sequence(self, game_state):
        remaining_locations = [
            location
            for location in FLEX_BUILD_SEQUENCE
            if self.get_structure_at(game_state, location) is None
        ]
        if not remaining_locations:
            return

        budget_units = int(game_state.get_resource(SP) * 2 + 1e-9)
        support_count, wall_count = self.choose_flex_plan(budget_units, len(remaining_locations))
        if support_count == 0 and wall_count == 0:
            return

        for location in remaining_locations[:support_count]:
            if game_state.attempt_spawn(SUPPORT, location) == 0:
                return

        support_end = support_count + wall_count
        for location in remaining_locations[support_count:support_end]:
            if game_state.attempt_spawn(WALL, location) == 0:
                return

    def choose_flex_plan(self, budget_units, remaining_slots):
        best_supports = 0
        best_walls = 0
        best_used = -1
        for supports in range(remaining_slots + 1):
            for walls in range(remaining_slots - supports + 1):
                used = supports * SUPPORT_WEIGHT + walls * WALL_WEIGHT
                ...
                if used > best_used:
                    ...
                    best_walls = walls
                elif used == best_used and supports > best_supports:
                    best_supports = supports
                    best_walls = walls
        return best_supports, best_walls

    def is_initial_layout_complete(self, game_state):
        return self.locations_match_type(game_state, WALL, EARLY_WALLS) and self.sequence_filled(
            game_state,
            FLEX_BUILD_SEQUENCE,
        )

    def sequence_filled(self, game_state, locations):
        for location in locations:
            if self.get_structure_at(game_state, location) is None:
                return False
        return True

    def upgrade_in_order(self, game_state, locations):
        for location in locations:
    ...
    def build_and_upgrade_support_queue(self, game_state):
        while True:
            built = self.build_next_support(game_state, SUPPORT_BUILD_UPGRADE_QUEUE)
            upgraded = self.upgrade_next_in_queue(game_state, SUPPORT_BUILD_UPGRADE_QUEUE)
            if not built and not upgraded:
                return

    def build_next_support(self, game_state, locations):
        for location in locations:
            if self.get_structure_at(game_state, location) is not None:
                continue
            return game_state.attempt_spawn(SUPPORT, location) > 0
        return False

    def upgrade_next_in_queue(self, game_state, locations):
        for location in locations:
            structure = self.get_structure_at(game_state, location)
            if structure is None or structure.upgraded:
                continue
